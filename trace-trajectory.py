...
def run_graphics(tTot, n=1, psW=False, pDriv=0.03, ts=1.7e-7, avg=0.41, dirname="sim"):
    ...
    while i < n:
        ...
        if b:
            ...
            if et != -1:
                nout += 1
                if len(exitTime) > 0 and et < min(exitTime):
                    traj = i
                exitTime.append(et)
    ...
    while t < dur:
        ...
        for k in range(n):
            try:
                ...
                if k == traj:
                    canvas.create_line(coords[k][0][t]+x1, coords[k][1][t]+y1, coords[k][0][t-1]+x1, coords[k][1][t-1]+y1, fill='cyan')
            except:
                continue
        canvas.update()
...
for j in range(n):
    ...
    while b:
        ...
        x, y, b, out0, out1, out2, out3, out4 = move(t, 1)
    print(len(x))
